...
import {
  moveInSchema as checklistSchema,
  generateMoveInDefaultValues,
} from "@agensy/config";
import {
  Card,
  CommonLoader,
...
export const MoveIn = () => {
  ...
  const {
    ...
    refetch,
  } = useGetChecklistForms("move_in", params.clientId!);
  ...
  useEffect(() => {
    if (
      ...
      const mergedData = { ...generateMoveInDefaultValues() };
  ...
  useEffect(() => {
    if (postStartCareChecklistMutation.status === "success") {
      toast.success(
        "Move In Checklist Successfully Saved",
        "The move in checklist information has been saved successfully."
      );
